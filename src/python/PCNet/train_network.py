...
import time
import pytorch_ssim
from src.python.PCNet import models, utils as ut
from src.python.My_PCNet.img_proc import threshold_im, center_crop as cc
from src.python.PCNet.utils import vis, plot_montage, append_data_point
...
def train_pcnet(model, train_data, valid_data, cfg):
    ...
    while iters < cfg.max_iters:
        ...
        warp_train_infer = torch.clamp(model.module.warp_only(prj_train_batch), -1, 1)
        ...
        if cfg.plot_on:
            ...
            # cp_sz = cfg.setup_info.classifier_crop_sz  # visualize center crop
            if iters % cfg.train_plot_rate == 0 or iters == cfg.max_iters - 1:
                vis_train_fig = plot_montage(prj_train_batch[vis_idx], cam_train_infer[vis_idx],
                                             warp_train_infer[vis_idx], cam_train_batch[vis_idx], win=vis_train_fig,
                                             title='[Train]' + title)
                append_data_point(iters, train_loss_batch.item(), vis_curve_fig, 'train_loss')
                append_data_point(iters, train_rmse_batch, vis_curve_fig, 'train_rmse')
...
def get_model_train_cfg(model_list, data_root=None, setup_list=None, device_ids=[0], center_crop=False, load_pretrained=False, plot_on=True,
    ...
    cfg_default.train_plot_rate    = 100
