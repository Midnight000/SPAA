...
def load_data(data_root, setup_name, input_size=None, compensation=False):
    ...
    prj_train_path = join(data_root , 'prj_DR_noise/train')
    prj_valid_path = join(data_root , 'prj_DR_noise/test')
    ...
    im_diff = ut.readImgsMT(cam_ref_path, index=[2], size=input_size) - ut.readImgsMT(cam_ref_path, index=[0], size=input_size)
    im_diff = im_diff.numpy().transpose((2, 3, 1, 0))
    prj_fov_mask = torch.zeros(cam_scene.shape)

    # threshold im_diff with Otsu's method
    mask_corners = [None] * im_diff.shape[-1]
    for i in range(im_diff.shape[-1]):
        im_mask, _, mask_corners[i] = img_proc.threshold_im(im_diff[:, :, :, i])
        prj_fov_mask[i, :, :, :] = ut.repeat_np(torch.Tensor(np.uint8(im_mask)).unsqueeze(0), 3, 0)

    prj_fov_mask = prj_fov_mask.bool()

    return cam_scene, cam_train, cam_valid, prj_train, prj_valid, prj_fov_mask, mask_corners[0], setup_info

def load_mask_info(data_root, setup_name, input_size=None, compensation=False):
    # data paths
    setup_path     = join(data_root , 'setups', setup_name)
    cam_ref_path   = join(setup_path, 'cam/raw/ref')
    print("Loading data from '{}'".format(setup_path))
    ...
    return prj_fov_mask, mask_corners[0], setup_info
...
def train_pcnet(model, train_data, valid_data, cfg):
    ...
    # learning rate drop scheduler
    w1_lr_scheduler = optim.lr_scheduler.MultiStepLR(w1_optimizer, milestones = [400, 1000],  gamma = cfg.lr_drop_ratio)
    w2_lr_scheduler = optim.lr_scheduler.MultiStepLR(w2_optimizer, milestones = [2500], gamma = cfg.lr_drop_ratio)
    warping_attention_lr_scheduler = optim.lr_scheduler.MultiStepLR(warping_attention_optimizer, milestones=[2500], gamma=cfg.lr_drop_ratio)
    shading_attention_lr_scheduler = optim.lr_scheduler.MultiStepLR(shading_attention_optimizer, milestones=[2500, 5000], gamma=cfg.lr_drop_ratio)
    s_lr_scheduler  = optim.lr_scheduler.MultiStepLR(s_optimizer , milestones = [2500, 5000], gamma = cfg.lr_drop_ratio)
    ...
    # Done training and save the last epoch model
    #     if iters % 5==0:
    #         attention2_weights = model.module.warping_net.attention1[1].weight.data.cpu().numpy()
    #         vis.heatmap(attention2_weights[:,:,0,0], opts={'title': 'Attention2 Weight'})
    ut.save_checkpoint(join(cfg.data_root, '../checkpoint'), model, title)
...
def train_eval_pcnet(cfg_default):
    ...
    for setup_name in cfg_default.setup_list:
        ...
        # image_pil = Image.fromarray((cam_mask[0].permute(1, 2, 0).numpy()*255).astype('uint8'))
        # image_pil.show()
