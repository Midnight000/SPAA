...
class Info_O_DDIMSampler(DDIMSampler):
    def __init__(self, use_timesteps, conf=None, **kwargs):
        super().__init__(
            use_timesteps=use_timesteps,
            conf=conf,
    ...
    def p_sample(
        ...
        **kwargs,
        ...
        with torch.enable_grad():
            ...
            if compensate:
                cam_desire = pred_x0
                cam_surf = model_kwargs["gt"]
                cam_desire = compen.back_prop_single((torch.ones_like(cam_desire) * 0.309).to(cam_desire.device),
                                                     cam_desire, cam_surf, "My_PCNet")
                prev_loss += loss_fn(cam_desire, pred_x0, ~mask).item()
            # directory = '/mnt/data/huang-lab/shipeng/celeb/copaint_copy/half/reverse/pred' + str(model_kwargs["image_name"])
            # make_dirs(directory)
            # tmp_pred = pred_x0
            # tmp_pred = ((tmp_pred + 1) * 127.5).clamp(0, 255).to(torch.uint8)
            ...
            for step in range(self.num_iteration_optimize_xt):
                ...
                if compensate:
                    ...
                    cam_desire = compen.back_prop_single((torch.ones_like(cam_desire) * 0.309).to(cam_desire.device),
                                                         cam_desire, cam_surf, "My_PCNet")
                    loss += loss_fn(cam_desire, pred_x0, ~mask).item()
                x_grad = torch.autograd.grad(
                    loss, x, retain_graph=False, create_graph=False
                )[0].detach()
                new_x = x - lr_xt * x_grad
